...
import pandas as pd
from batdongsan import navigateToWeb, getJsonHouses
import json
import random
import os
import time
...
# Checkpoint file
CHECKPOINT_FILE = "./kafka_spark/json/checkpoint.txt"
    ...
    for i in range(max(start_page, last_page), end_page):
        ...
        while retries > 0:
            try:
                ...
                print(f"Retrieved {len(houses_json)} records from page {i + 1}.")

                # Send data to Kafka
                for record in houses_json:
